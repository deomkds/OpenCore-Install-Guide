# Ryzen e Threadripper (17h e 19h)
...
* [Patches de Kernel para AMD](https://github.com/AMD-OSX/AMD_Vanilla/tree/opencore)
  ...
  * [Ryzen/Threadripper(17h/19h)](https://github.com/AMD-OSX/AMD_Vanilla/tree/opencore/17h_19h) (Suporta macOS 10.13 High Sierra, 10.14 Mojave e 10.15 Catalina).
...
* **SetupVirtualMap**: YES
  ...
  * Esta *quirk* deve ser desativada em placas-mãe B550, A520 e TRX40.
* **SyncRuntimePermissions**: YES
...
* [Ryzen/Threadripper(17h/19h)](https://github.com/AMD-OSX/AMD_Vanilla/tree/opencore/17h_19h) (macOS 10.13 High Sierra, 10.14 Mojave e 10.15 Catalina).
...
**Observação especial para usuários da CPU 3990X**: Atualmente, o macOS não suporta mais do que 64 *threads* no *kernel*. Caso tenha mais, ocorrerá uma *kernel panic*. A CPU 3990X possui um total de 128 *threads* e então precisa que metade deles sejam desligados. Nessas situações, recomenda-se desligar o *hyper-threading* na BIOS.

### Enable

| Nome em Inglês | Nome em Português | Oservação |
| :--- | :--- | :--- |
| Above 4G decoding | NA | **Precisa estar ligado.** Se não puder encontrar essa opção, adicione `npci=0x2000` aos argumentos de inicialização (*boot-args*). Não mantenha essa opção da BIOS e o argumento de *boot* ligados ao mesmo tempo!</br>Ativar essa opção pode quebrar certos drivers (ex.: Ethernet) ou causar falhas de inicialização em outros sistemas operacionais em placas-mãe Gigabyte/Aorus ou AsRock. Se isso acontecer, desative esta opção e, em vez dela, opte por utilizar a `npci`.</br>Em BIOS mais recentes (a partir de 2020), ativar o Above4G pode desbloquear a opção Resizable BAR Support em algumas placas-mãe X570 ou mais novas. Tenha certeza de **desativar** a opção Resizable BAR Support, em vez de mantê-la em `Auto`. |
