...
| SetupVirtualMap | YES | Observe que BIOS da Asus mais novas (v3006+) não inciarão caso esta *quirk* esteja ativada. |
...
* **DevirtualiseMmio**: YES
  * Reduz a quantidade de memória reservada, expande as opções de valores do `slide=N` e ajuda bastante a corrigir problemas de alocação de memória em placas-mãe Z390. Exige também `ProtectUefiServices` em computadores com processadores Ice Lake e em placas-mãe Z390 com processadores Coffee Lake.
...
* **SetupVirtualMap**: YES
  * Corrige as chamadas de `SetVirtualAddresses` para endereços virtuais. Desnecessário em computadores com CPUs Skylake ou mais recente. Alguns firmwares, como os da Gigabyte, talvez ainda exijam isso, podendo ter *kernel panic* caso não esteja presente.
  * Observe que BIOS da Asus mais novas (v3006+) não iniciarão se esta opção estiver ligada.
* **SyncRuntimePermissions**: YES
...
Necessário para "falsificar" CPUs não suportadas. Porém, tanto o Skylake-X quanto o Cascade Lake-X possuem o mesmo ID de CPU dos  chips Xeon W que são usados no iMac Pro. Então deixe essa seção vazia.
...
Neste exemplo de Skylake-X, a SMBIOS do iMacPro1,1 será utilizada.
