...
| Suporte | Versão |
| :--- | :--- |
| Suporte Inicial no macOS | OS X 10.11, El Capitan |
...
| **[SSDT-RTC0-RANGE](https://deomkds.github.io/Getting-Started-With-ACPI/)** | Necessária para ativar o antigo relógio RTC no macOS. Também é necessário para todos os usuários do macOS 11 Big Sur para garantir que o dispositivo RTC seja compatível. Consulte o guia [Primeiros Passos com a ACPI](https://dortania.github.io/Getting-Started-With-ACPI/) para obter mais informações. |
...
Necessário para "falsificar" CPUs não suportadas e ativar o gerenciamento de energia em processadores Haswell-E e Broadwell-E.
...
* **CpuidData**: `D4060300 00000000 00000000 00000000`
  * Entrada de ID de CPU falso.
* **CpuidMask**: `FFFFFFFF 00000000 00000000 00000000`
  * Máscara para o ID de CPU falso.
* **DummyPowerManagement**: No
  * Desabilita a `AppleIntelCPUPowerManagement.kext`. Necessário somente em computadores com CPUs AMD.
* **MinKernel**: deixe vazio.
  * Versão mínima do *kernel* no qual os patches acima serão injetados. Se nenhum valor for especificado, será aplicado em todas as versões do macOS. Veja a tabela abaixo para todos os possíveis valores.
  * Ex.: `12.00.00` para o OS X 10.8 Mountain Lion.
* **MaxKernel**: deixe vazio.
  * Versão máxima do *kernel* no qual os patches acima serão injetados. Se nenhum valor for especificado, será aplicado em todas as versões do macOS. Veja a tabela abaixo para todos os possíveis valores.
  * Ex.: `11.99.99` para o OS X 10.7 Lion.
...
| :--- | :--- | :--- |
| AppleCpuPmCfgLock | NO | Necessário se estiver executando o OS X 10.10 Yosemite ou mais antigo e não puder desabilitar o `CFG-Lock` na BIOS. |
| AppleXcpmCfgLock | YES | Desnecessário se o `CFG-Lock` estiver desabilitado na BIOS. |
...
Reescreve as variáveis NVRAM na marra. Observe que `Add` **não sobrescreve** valores que já estejam presentes na NVRAM. Então, valores como os `boot-args` devem ser deixados como estão. Devido a problemas de NVRAM em placas X99, será preciso alterar as seguintes opções:
...
* **LegacyEnable**: YES
  * Permite que a NVRAM seja armazenada no arquivo `nvram.plist`. Necessário em computadores que não possuem NVRAM nativa como X99.

* **LegacyOverwrite**: YES
  * Permite sobrescrever variáveis de firmware a partir do arquivo `nvram.plist`. Necessário somente em computadores que não possuem NVRAM nativa como X99.

* **LegacySchema**
  * Usado para atribuir variáveis NVRAM. Usado com o `LegacyEnable` configurado para YES.

* **WriteFlash**: NO
  * Ativa a função de escrever em memória *flash* para todas as variáveis adicionadas. Incompatível com NVRAM emulada.
...
* No OS X 10.10 Yosemite ou mais antigo, será necessário ativar a opção `AppleCpuPmCfgLock` também.
