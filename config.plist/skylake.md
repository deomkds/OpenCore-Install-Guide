...
![DeviceProperties](../images/config/config.plist/skylake/DeviceProperties.png)
...
| :--- | :--- |
| 00001219 | Usado quando a GPU integrada de desktops é usada para exibir as imagens no monitor. |
| 01001219 | Usado quando a GPU integrada de desktop é utilizada somente para tarefas de computação e não exibe imagens em um monitor. |
...
* **Observação**: framebuffers *headless* (nos quais a GPU dedicada é responsável por exibir as imagens) não precisam das propriedades `framebuffer-patch-enable`, `framebuffer-stolenmem` e `framebuffer-fbmem`.

E uma observação extra para usuários de placas P530: a GPU integrada delas não é suportada nativamente, então será necessário adicionar a seguinte propriedade:

| Chave | Tipo | Valor |
| :--- | :--- | :--- |
| device-id | Data | 1B190000 |

Assim que estiver terminado, deverá ficar parecido com isso:
...
*Isto é um exemplo para um desktop HD P530 sem GPU dedicada e que não possui opção para configurar a memória da GPU integrada na BIOS.*
...
Neste exemplo de Skylake, será utilizada a SMBIOS do iMac17,1.
