# Desktop Lynnfield e Clarkdale

| Suporte | Versão |
| :--- | :--- |
| Suporte Inicial no macOS | Mac OS X 10.6.3 Snow Leopard |
| Observação 1 | Não há suporte para GPUs integradas Iron Lake. |
| Observação 2 | A maioria das placas-mãe Lynnfield e Clarkdale não suportam UEFI. |
...
Por padrão, a `Sample.plist` apresenta esta seção já configurada para GPUs integradas e para áudio. Como os drivers da Apple para GPUs integradas Iron Lake não possuem suporte para o hardware, o PciRoot `PciRoot(0x0)/Pci(0x2,0x0)` pode ser removido da seção `Add`. Para o áudio, o layout será configurado na seção de argumentos de inicialização (*boot-args*), então a remoção do `PciRoot(0x0)/Pci(0x1b,0x0)` também é recomendada tanto da seção `Add` quanto da seção `Block`.
...
| SMBIOS | Hardware |
| :--- | :--- |
| iMac11,1 | SMBIOS para Lynnfield. |
