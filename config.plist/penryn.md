...
| Suporte | Versão |
| :--- | :--- |
| Suporte Inicial no macOS: Penryn | Mac OS X 10.4.10 Tiger |
| Suporte Final no macOS: Penryn | macOS 10.13.6 High Sierra |
| Observação 1 | O suporte à GPU integrada não será abordado neste guia. Acesse o guia [Patches de GMA](https://deomkds.github.io/OpenCore-Post-Install/gpu-patching/) para mais informações. |
| Observação 2 | O conjunto de instruções SSE4 é necessário para iniciar o macOS 10.12 Sierra ou mais novo, então CPUs Conroe e mais antigas não são suportadas. |
...
![ACPI](../images/config/config-legacy/penryn-acpi.png)
...
Esta seção bloqueia o carregamento de certas tabelas da ACPI. Pode ser ignorado por enquanto.
...
| Legado | UEFI |
| :--- | :--- |
| ![Booter](../images/config/config-legacy/booter-duetpkg.png) | ![Booter](../images/config/config-universal/aptio-iv-booter-sl.png) |
...
| :--- | :--- | :--- |
| AvoidRuntimeDefrag | NO | O macOS 11 Big Sur talvez exija que esta *quirk* permaneça ativada. |
...
* **AvoidRuntimeDefrag**: NO
  ...
  * O macOS 11 Big Sur, no entanto, exige que a tabela APIC esteja presente, caso contrário, *kernel panics* precoces podem acontecer. Portanto, essa *quirk* é recomendada para essas versões do macOS.
...
![Kernel](../images/config/config-legacy/penryn-kernel.png)
...
| PanicNoKextDump | YES | Desnecessário no macOS 10.12 Sierra ou mais antigo. |
| PowerTimeoutKernelPanic | YES | Desnecessário no macOS 10.14 Mojave ou mais antigo. |
...
* **AppleCpuPmCfgLock**: NO
  * A trava de CFG não existe no Penryn, então essa *quirk* é desnecessária.
* **AppleXcpmCfgLock**: NO
  * A trava de CFG não existe no Penryn, então essa *quirk* é desnecessária.
* **CustomSMBIOSGuid**: NO
...
* **XhciPortLimit**: NO
  * Isto é, na verdade, o patch que corrige o limite de 15 portas USB. No entanto, só é relevante em computadores com controladores XHCI (USB 3.0).
...
| boot-args | Descrição |
| :--- | :--- |
| **nvda_drv_vrl=1** | Usado para ativar os Web Drivers da Nvidia em placas Maxwell e Pascal no macOS 10.12 Sierra e 10.13 High Sierra. macOS 10.12 Sierra ou mais antigos podem usar `nvda_drv=1` |
...
Para este exemplo de Penryn, será utilizada a SMBIOS do iMac10,1. Isso é intencional, por questões de compatibilidade. Existem quatro principais SMBIOS usadas em computadores antigos:

| SMBIOS | Hardware | Suporte de SO |
| :--- | :--- | :--- |
| iMac4,1 | SMBIOS para Yonah (32 bits) | Do Mac OS X 10.4 Tiger ao 10.6.8 Snow Leopard. |
| iMac7,1 | SMBIOS para Conroe (64 bits, SSE3) | Do Mac OS X 10.4 Tiger ao OS X 10.11.6 El Capitan. |
| iMac10,1 | SMBIOS para Penryn (64 bits, SSE4) | Do Mac OS X 10.6 Snow Leopard ao macOS 10.13.6 High Sierra. |
| MacPro6,1 | SMBIOS para macOS 10.14 Mojave ou mais novos. | Do OS X 10.9 Mavericks ao mais atual. |

* Se planeja executar o macOS 10.14 ou mais recente, é recomendado usar a SMBIOS do MacPro6,1. No entanto, será necessário usar a [telemetrap.kext](https://forums.macrumors.com/threads/mp3-1-others-sse-4-2-emulation-to-enable-amd-metal-driver.2206682/page-4?post=28447707#post-28447707) (em inglês) para corrigir problemas de instalação.
...
| Quirk | Ativada | Observação |
| :--- | :--- | :--- |
| IgnoreInvalidFlexRatio | NO | Ative somente se tiver um firmware UEFI. |
...
* **IgnoreInvalidFlexRatio**: NO
  ...
  * Relevante somente em firmwares UEFI.
