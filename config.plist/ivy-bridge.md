...
| **[SSDT-IMEI](https://deomkds.github.io/Getting-Started-With-ACPI/)** | Necessário para adicionar o dispositivo IMEI faltante em placas-mãe série 6 que utilizam CPUs Ivy Bridge. |
...
| :--- | :--- |
| 0A006601 | Usado quando a GPU integrada é responsável por exibir as imagens no monitor.  |
| 07006201 | Usado quando a GPU integrada é responsável somente por tarefas de computação e não exibe imagens em um monitor. |
...
*Isto é um exemplo de um desktop com Intel HD Graphics 4000 sem GPU dedicada.*

:::

::: tip PciRoot(0x0)/Pci(0x16,0x0)

Isso é necessário ao combinar uma CPU Ivy Bridge com placas-mãe série 6, como H61, B65, Q65, P67, H67, Q67 e Z68. Tem o propósito de "falsificar" o dispositivo IMEI de forma que ele seja suportado. Essa propriedade é necessária independentemente da existência ou não de uma SSDT-IMEI.

| Chave | Tipo | Valor |
| :--- | :--- | :--- |
| device-id | Data | 3A1E0000 |

**Observação**: essa propriedade não é necessária em placas-mãe série 7, como B75, Q75, Z75, H77, Q77 e Z77.
...
Para este exemplo de Ivy Bridge, será utilizada a SMBIOS do iMac13,2. Isso é intencional, por questões de compatibilidade. Existem três principais SMBIOS usadas em computadores com CPUs Ivy Bridge:

| SMBIOS | Hardware |
| :--- | :--- |
| iMac13,1 | Usado quando a GPU integrada é responsável por exibir as imagens no monitor. |
| iMac13,2 | Usado quando a GPU integrada é responsável somente por tarefas de computação e não exibe imagens em um monitor. |

**Observação**: As SMBIOS acima suportam somente o macOS 10.15 Catalina e anteriores. Para os casos em que for necessário executar o macOS 11 Big Sur, veja abaixo:

::: details SMBIOS do macOS 11 Big Sur

Observe que escolher uma das SMBIOS abaixo para o macOS 10.15 Catalina ou mais antigo não é recomendado, pois o Gerenciamento de Energia e outras funções podem parar de funcionar.

| SMBIOS | Hardware |
| :--- | :--- |
| iMac14,4 | Usado quando a GPU integrada é responsável por exibir as imagens no monitor. |
| iMac15,1 | Usado quando a GPU integrada é responsável somente por tarefas de computação e não exibe imagens em um monitor. |
