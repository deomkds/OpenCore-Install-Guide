# Notebook Sandy Bridge

| Suporte | Versão |
| :--- | :--- |
| Suporte Inicial no macOS | Mac OS X 10.6.7 Snow Leopard |
| Observação 1 | A GPU integrada das CPUs Sandy Bridge só possui suporte oficial até o macOS 10.13 High Sierra. |
| Observação 2 | A maioria das placas Sandy Bridge não suportam UEFI. |
...
| **[SSDT-IMEI](https://deomkds.github.io/Getting-Started-With-ACPI/)** | Necessário para adicionar o dispositivo IMEI faltante em CPUs Sandy Bridge com pĺacas-mãe série 7. |
...
1. Ao criar a primeira `config.plist`, configure somente a propriedade `AAPL,snb-platform-id`. É o suficiente para iniciar o instalador.

| AAPL,snb-platform-id | Tipo | Observação |
| ------------------- | ---- | ------- |
| **00000100** | Notebook | Em notebooks, use este valor. |
| **10000300** | NUC | Nos NUCs da Intel, use este valor. |
...
* As GPUs integradas Intel HD Graphics 2000 não são suportadas.
...
Alguns notebooks dessa era vieram com um chipset misturado, usando CPUs Sandy Bridge em chipsets Ivy Bridge. Isso cria problemas no macOS, já que ele espera encontrar um certo ID de [dispositivo IMEI](https://en.wikipedia.org/wiki/Intel_Management_Engine) (em inglês). Quando ele não encontra esse ID, o sistema trava na inicialização porque os drivers de GPUs integradas da Apple exigem a presença de um dispositivo IMEI. Para corrigir isso, será necessário falsificar o ID do dispositivo IMEI nessas configurações.

* Para saber se este problema afeta você, verifique se o modelo da CPU é um Intel Core ix-2xxx e se o chipset é um Hx7x (por exemplo, um notebook com HM75 ou HM77 que utilize uma CPU Core i3-2310M) usando ferramentas como o AIDA64 ou o HWiNFO.
...
| XhciPortLimit | YES | Desnecessário caso o notebook não possua portas USB 3.0. |
...
Neste exemplo de Sandy Bridge, a SMBIOS do MacBookPro8,1 será utilizada. Isso é intencional, por questões de compatibilidade. É mais ou menos assim:

| SMBIOS | Tipo de CPU | Tipo de GPU | Tamanho do Monitor |
| :--- | :--- | :--- | :--- |
| MacBookAir4,1 | Dual Core 17w | GPU integrada: HD 3000 | 11" |
| MacBookAir4,2 | Dual Core 17w | GPU integrada: HD 3000 | 13" |
| MacBookPro8,1 | Dual Core 35w | GPU integrada: HD 3000 | 13" |
| MacBookPro8,2 | Quad Core 45w (Alto Desempenho) | GPU integrada: HD 3000 + 6490M | 15" |
| MacBookPro8,3 | Quad Core 45w (Alto Desempenho) | GPU integrada: HD 3000 + 6750M | 17" |
| MacMini5,1 | Dual Core NUC | GPU integrada: HD 3000 | N/A |
| MacMini5,3 | Quad Core NUC | GPU integrada: HD 3000 | N/A |
...
* `HfsPlus.efi`
* `OpenRuntime.efi`
* OpenUsbKbDxe.efi (caso o firmware não seja UEFI)
