...
| **[SSDT-RHUB](https://deomkds.github.io/Getting-Started-With-ACPI/)** | Necessária para corrigir erros de Root-device em muitos notebooks com CPUs Ice Lake. |
...
* **DevirtualiseMmio**: YES
  * Reduz a quantidade de memória reservada, expande as opções de valores do `slide=N` e ajuda bastante a corrigir problemas de alocação de memória em placas Z390. Também exige `ProtectUefiServices` em computadores com CPUs Ice Lake e placas Z390 com CPUs Coffee Lake.
...
* **ProtectUefiServices**: YES
  ...
  * Em placas Z390, **ative esta _quirk_.**
...
* **SetupVirtualMap**: NO
  * Corrige as chamadas de `SetVirtualAddresses` para endereços virtuais, mas pode causar *kernel panics* em computadores com CPUs Ice Lake.
* **SyncRuntimePermissions**: YES
...
* **Port Count**
  * O número de monitores suportados.
...
| AAPL,ig-platform-id | Nº de Portas | Observação |
| ------------------- | ---------- | ------- |
| **0000528A** | 6 | Valor G4/G7 recomendado. |

#### Observações Sobre a Configuração

* Em alguns casos, quando não é possível configurar a quantidade de memória DVMT pré-alocada para 256MB ou mais nas configurações da UEFI, pode ocorrer um *kernel panic*. Geralmente, é configurado para 32MB por padrão. Nesses casos, basta adicionar os valores corretos nas propriedades da GPU integrada.
...
| **-igfxcdc** | Resolve *kernel panics* de Clock ID em CPUs Ice Lake. Recomendado para todos os usuários. |
| **-igfxdvmt** | Similar ao de cima. Resolve um problema em alguns firmwares que possuem 60MB de memória reservada para a GPU integrada, que pode causar *kernel panics*. |
...
Neste exemplo de Ice Lake, a SMBIOS do MacBookAir9,1 será utilizada. Isso é intencional, por questões de compatibilidade. É mais ou menos assim:

| SMBIOS | Tipo de CPU | Tipo de GPU | Tamanho do Monitor | Touch ID |
| :--- | :--- | :--- | :--- | :--- |
| MacBookAir9,1 | Dual/Quad Core 12w | GPU integrada: G4/G7 | 13" | Sim |
| MacBookPro16,2 | Quad Core 28w | GPU integrada: G4/G7 | 13" | Sim |
...
| DVMT Pre-Allocated (iGPU Memory): 256MB | DVMT Pré-alocado (memória da GPU integrada): 256MB | |
